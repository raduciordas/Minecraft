...
export function stepBody(body: Body, world: World, dt: number, stepHeight = 0): void {
  ...
  if (collidesAt(world, newX, body.y, body.z, halfW, height)) {
    if (stepHeight > 0 && !collidesAt(world, newX, body.y + stepHeight, body.z, halfW, height)) {
      body.y += stepHeight;
      steppedUp = true;
    } else {
      if (body.vx > 0) {
  ...
  if (collidesAt(world, body.x, body.y, newZ, halfW, height)) {
    if (!steppedUp && stepHeight > 0 && !collidesAt(world, body.x, body.y + stepHeight, newZ, halfW, height)) {
      body.y += stepHeight;
    } else {
      if (body.vz > 0) {
