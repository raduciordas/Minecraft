...
export function buildPlayerAvatar(name: string, color: PlayerColor): PlayerAvatarModel {
  ...
  box(group, 0.52, 0.09, 0.3, tintColor(shirt, 0.24), 0, 1.33, 0);
  box(group, 0.52, 0.1, 0.3, shadeColor(shirt, 0.6), 0, 0.75, 0);
  const head = box(group, 0.45, 0.45, 0.45, skin, 0, 1.62, 0);
  box(head, 0.47, 0.12, 0.47, shadeColor(shirt, 0.48), 0, 0.2, 0.01);
  for (const side of [-1, 1]) {
    box(head, 0.1, 0.09, 0.025, 0xf7f4e9, side * 0.11, 0.04, -0.24);
    box(head, 0.04, 0.055, 0.03, shadeColor(shirt, 0.4), side * 0.11, 0.04, -0.255);
  }
  box(head, 0.12, 0.035, 0.025, 0x8f4f45, 0, -0.1, -0.245);
  ...
  for (const side of [-1, 1]) {
    ...
    const arm = new THREE.Mesh(armGeo, voxelMaterial(shirt));
    arm.position.set(side * 0.33, 1.38, 0);
    group.add(arm);
    arms.push(arm);
  }
  for (const side of [-1, 1]) {
    legs.push(leg(group, 0.18, 0.7, pants, side * 0.14, 0.7, 0));
    box(group, 0.2, 0.12, 0.3, shadeColor(pants, 0.55), side * 0.14, 0.06, -0.05);
  }
