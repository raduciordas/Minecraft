...
export interface PickupGrantedEvent {
...
interface Handlers {
  ...
  pickupGranted: (e: PickupGrantedEvent) => void;
}
...
export class NetworkClient {
  ...
  connect(url: string, name: string): Promise<InitPayload | null> {
    return new Promise((resolve) => {
      ...
      ws.addEventListener('message', (ev) => {
        ...
        else if (msg.type === 'pickupItem') this.handlers.pickupItem?.(msg.id as string);
        else if (msg.type === 'pickupGranted') this.handlers.pickupGranted?.(msg as unknown as PickupGrantedEvent);
      });
  ...
  sendDropItem(item: DroppedItemData): void {
    if (!this.connected || !this.ws) return;
    this.ws.send(JSON.stringify({ type: 'dropItem', ...item }));
  }
